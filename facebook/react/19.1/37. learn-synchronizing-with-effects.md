...
The user visits `<ChatRoom roomId="general" />`. Let’s [mentally substitute](about:/learn/state-as-a-snapshot#rendering-takes-a-snapshot-in-time) `roomId` with `'general'`:
